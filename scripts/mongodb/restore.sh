...
if [ $# -lt 3 ]; then
    echo "Usage: "
    echo "     ./restore.sh <docker_name> <database_name> <restore_from_path>"
    echo "Options: "
    echo "     docker_name: the mongodb docker container name or id "
    echo "     database_name: the mongodb database to restore "
    echo "     restore_from_path: the path including the mongodb dump files "
    exit 1;
fi
...
docker exec $DOCKER rm -fr /tmp/backup/$BACKUP_PATH
